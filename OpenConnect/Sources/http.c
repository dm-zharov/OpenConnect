...
struct oc_text_buf *oc_buf_alloc(void)
{
...
int oc_buf_error(struct oc_text_buf *buf)
{
	return buf ? buf->error : -ENOMEM;
}

int oc_buf_free(struct oc_text_buf *buf)
{
	int error = oc_buf_error(buf);
			    ...
			    struct oc_text_buf *hdrbuf, int allow_cont)
{
	...
	while (1) {
		...
		if (eol && c != ' ' && c != '\t') {
			*nextchar = c;
			return oc_buf_error(hdrbuf);
		}
		eol = 0;

	skip_first:
		if (c == '\n') {
			if (!oc_buf_error(hdrbuf) && hdrbuf->pos &&
...
{
	struct oc_text_buf *hdrbuf = oc_buf_alloc();
	char nextchar = 0;
	 ...
	 * later without checking (for anything except oc_buf_error(hdrbuf), which is
...
{
	struct oc_text_buf *line = oc_buf_alloc();
	int i, j;
		     ...
		     char **form_buf, int fetch_redirect)
{
	...
	if (request_body_type && oc_buf_error(request_body))
		return oc_buf_error(request_body);
...
static int process_http_proxy(struct openconnect_info *vpninfo)
{
 ...
 retry:
	...
	if (oc_buf_error(reqbuf))
		return oc_buf_free(reqbuf);
