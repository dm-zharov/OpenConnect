...
static void buf_fill_eap_len(struct oc_text_buf *buf, int ofs)
{
	...
	if (ofs >= 0 && !oc_buf_error(buf) && buf->pos >= ofs + 4)
...
{
	if (oc_buf_error(buf) || buf->pos < 16) {
		vpn_progress(vpninfo, PRG_ERR,
...
{
	struct oc_text_buf *buf = oc_buf_alloc();
	const char *pretty;
	int i;
				      ...
				      int sessions, unsigned char *eap)
{
	...
	void *p;
	int l;
	struct oc_text_buf *form_msg = oc_buf_alloc();
	char tmbuf[80];
	struct tm tm;
...
static int pulse_authenticate(struct openconnect_info *vpninfo, int connecting)
{
 ...
 out:
	...
	oc_buf_free(vpninfo->ttls_pushbuf);
...
int pulse_eap_ttls_send(struct openconnect_info *vpninfo, const void *data, int len)
{
	...
	if (!buf) {
		buf = vpninfo->ttls_pushbuf = oc_buf_alloc();
		if (!buf)
			return -ENOMEM;
	}
...
int pulse_eap_ttls_recv(struct openconnect_info *vpninfo, void *data, int len)
{
	...
	if (!vpninfo->ttls_recvlen) {
		...
		if (oc_buf_error(pushbuf))
			return oc_buf_free(pushbuf);
		...
		if (pushbuf->pos > TTLS_MAXFRAG + 0x1a) {
			struct oc_text_buf *frag = oc_buf_alloc();
			uint8_t flags = TTLS_MOREFRAGS | TTLS_LENGTH;
			uint32_t remaining;
			char *msg;

			if (oc_buf_error(frag)) {
				oc_buf_free(pushbuf);
				return oc_buf_free(frag);
			}
